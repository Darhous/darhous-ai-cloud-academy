...
| ID | Area | Severity | Type | Route/file | Symptom | Root cause | User impact | Recommended fix | Suggested phase | Dependencies | Production blocker |
|----|------|----------|------|------------|---------|------------|-------------|-----------------|-----------------|--------------|-------------------|
| FPA-001 | Landing | Critical | UX | `Premium3DShowcaseCarousel.tsx` | Owner sees no layered/3D cards | Component never imported on homepage | Expected premium effect missing | Mount below hero or replace with scroll-stack section | L1 | Design approval | yes |
| FPA-002 | Landing | Critical | UX | `HomepageClient.tsx` | No scroll-stacked card effect | Feature never implemented | Reports promise ≠ live UX | Implement `ScrollStackPortalShowcase` or approved equivalent | L1 | FPA-001 decision | yes |
...
| FPA-007 | Landing | High | UX | `SmartPlatformTour.tsx` | Tour never appears | Auto-open commented Phase 5D | Onboarding feature dead | Add explicit trigger button | L1 | — | no |
| FPA-008 | Landing | High | UX | `PortalGrid.tsx` / `portals.ts` | Cards hard to notice | Below fold + subtle glass | Users miss portals | Move showcase up; stronger contrast | L1 | — | no |
...
| FPA-011 | Brand | High | SEO | layout, manifest, certs | Mixed Darhous/NexaLearn | Partial Phase 5G | Share/install/certs inconsistent | Complete brand stack | B1 | — | yes |
...
| FPA-021 | Landing | Medium | UX | `CinematicIntro.tsx` | Intro skipped | sessionStorage | First-run only | Optional settings link to replay | L1 | — | no |
...
| FPA-026 | Architecture | Medium | architecture | Showcase carousels | Dead code 573 lines | Never integrated | Maintenance burden | Mount or delete | L1 | — | no |
...
| FPA-050 | Homepage | Low | SEO | `[locale]/page.tsx` metadata | Darhous not NexaLearn | Not updated | SEO brand leak | Update metadata | B1 | — | no |
...
| FPA-062 | OG | Low | SEO | og-image.svg | Old Darhous art | Not regenerated | Share preview old | New OG asset | B1 | — | no |
...
Note: FPA-001 and FPA-002 counted separately; executive summary rounds Critical to 8 including certificate brand (FPA-011/012 grouped in summary).
