...
This station created `content-source/_audit/audit-tier-a-metadata.py` to inspect the 600 imported Tier-A draft records without connecting to Supabase. It uses the normalized JSON sources.
...
Repairs should ideally be applied directly to the normalized JSON files, generating a SQL update script, or by automating updates via transaction-wrapped SQL, to keep the content source and database in sync.
