/**
 * GET    /api/admin/ai-paths/[id]  — fetch single roadmap
 * PATCH  /api/admin/ai-paths/[id]  — update fields / change status
 * DELETE /api/admin/ai-paths/[id]  — hard-delete roadmap
 */
import "server-only";
...
const TEXT_FIELDS = [
  ...
  "level", "outcome", "outcome_ar", "icon", "color", "status",
];
const NUMBER_FIELDS = ["total_weeks", "sort_order"];
const JSONB_FIELDS = ["nodes"];
...
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  const { data, error } = await admin.from("ai_paths").select("*").eq("id", id).single();
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  for (const key of TEXT_FIELDS) {
    if (key in body) updates[key] = body[key] === "" && key !== "status" ? "" : body[key];
  }
  ...
  const { data, error } = await admin.from("ai_paths").update(updates).eq("id", id).select("id,title_ar").single();
...
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  ...
  const { error } = await admin.from("ai_paths").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
